...
import {insertEmbeddingWithPost} from "~/app/newpost/embedding";
...
export default function ClientSide() {
  ...
  async function createEmbedding(post_id: number) {
    void await insertEmbeddingWithPost(content, post_id)
  }
  ...
  const createPost = api.posts.create.useMutation({
    onSuccess: (data) => {
      ...
      void createEmbedding(Number(data[0]?.id))
      setContent("");
      setImageUrls([]);
      ...
      <SignedIn>
        ...
        >
          ...
          >
            ...
            >
              <div>{blockSubmit ? "Image uploading..." : "Create post"}</div>
            </button>
          </div>
        </div>
      </SignedIn>
