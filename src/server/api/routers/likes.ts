    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (user) {
        const previousLike = await ctx.db.query.likes.findFirst({
          where: and(eq(likes.post_id, input.post_id),eq(likes.user_id, user.id))
        });
        if (previousLike) {
          await ctx.db.delete(likes).where(and(eq(likes.post_id, input.post_id),eq(likes.user_id, user.id)));
          return "Unliked"
        }
        await ctx.db
          .insert(likes)
